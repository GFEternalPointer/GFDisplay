...
#import "GFDisplayViewController.h"
#import "GFDisplayTitleLabel.h"
#import "GFDisplayViewHeader.h"
#import "UIView+GFFrame.h"
#import "GFFlowLayout.h"

static NSString * const ID = @"CONTENTCELL";

@interface GFDisplayViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
{
    UIColor *_norColor;
    UIColor *_selColor;
}
...
/**
 颜色渐变样式
 */
@property (nonatomic, assign) GFTitleColorGradientStyle titleColorGradientStyle;
...
@implementation GFDisplayViewController
...
{
    // 初始化标题高度
    _titleHeight = GFTitleScrollViewH;
...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        // 创建布局
        GFFlowLayout *layout = [[GFFlowLayout alloc] init];
...
- (void)setIsShowTitleScale:(BOOL)isShowTitleScale
{
    if (_isShowUnderLine) {
        // 抛异常
        NSException *excp = [NSException exceptionWithName:@"GFDisplayViewControllerException" reason:@"字体放大效果和角标不能同时使用。" userInfo:nil];
        [excp raise];
    }
...
- (void)setIsfullScreen:(BOOL)isfullScreen
{
    ...
    self.contentView.frame = CGRectMake(0, 0, GFScreenW, GFScreenH);
...
// 一次性设置所有颜色渐变属性
- (void)setUpTitleGradient:(void(^)(GFTitleColorGradientStyle *titleColorGradientStyle,UIColor **norColor,UIColor **selColor))titleGradientBlock;
{
    ...
    if (_titleColorGradientStyle == GFTitleColorGradientStyleFill && _titleWidth > 0) {
        @throw [NSException exceptionWithName:@"GF_ERROR" reason:@"标题颜色填充不需要设置标题宽度" userInfo:nil];
    }
}
...
- (void)setUpTitleScale:(void(^)(CGFloat *titleScale))titleScaleBlock
{
    ...
    if (_isShowUnderLine) {
        @throw [NSException exceptionWithName:@"GF_Error" reason:@"当前框架下标和字体缩放不能一起用" userInfo:nil];
    }
...
- (void)viewDidLayoutSubviews
{
    ...
    if (_isInitial == NO) {
        ...
        CGFloat titleY = self.navigationController.navigationBarHidden == NO ?GFNavBarH:statusH;
        ...
        if (_isfullScreen) {
            ...
            // 顶部标题View尺寸
            self.titleScrollView.frame = CGRectMake(0, titleY, GFScreenW, self.titleHeight);
        ...
        if (self.contentView.frame.size.height == 0) {
            self.contentView.frame = CGRectMake(0, titleY, GFScreenW, GFScreenH - titleY);
        }

        // 顶部标题View尺寸
        self.titleScrollView.frame = CGRectMake(0, 0, GFScreenW, self.titleHeight);

        // 顶部内容View尺寸
        CGFloat contentY = CGRectGetMaxY(self.titleScrollView.frame);
        CGFloat contentH = self.contentView.gf_height - contentY;
        self.contentScrollView.frame = CGRectMake(0, contentY, GFScreenW, contentH);
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (_isInitial == NO) {
        ...
        if (_titleColorGradientStyle == GFTitleColorGradientStyleFill || _titleWidth == 0) { // 填充样式才需要这样
...
- (void)setUpTitleWidth
{
    ...
    for (NSString *title in titles) {
        ...
        if ([title isKindOfClass:[NSNull class]]) {
            // 抛异常
            NSException *excp = [NSException exceptionWithName:@"GFDisplayViewControllerException" reason:@"没有设置Controller.title属性，应该把子标题保存到对应子控制器中" userInfo:nil];
            [excp raise];
    ...
    CGFloat titleMargin = (GFScreenW - totalWidth) / (count + 1);
...
- (void)setUpAllTitle
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        UILabel *label = [[GFDisplayTitleLabel alloc] init];
        ...
        if (_titleColorGradientStyle == GFTitleColorGradientStyleFill || _titleWidth == 0) { // 填充样式才需要
    ...
    _contentScrollView.contentSize = CGSizeMake(count * GFScreenW, 0);

}

#pragma mark - 标题效果渐变方法
// 设置标题颜色渐变
- (void)setUpTitleColorGradientWithOffset:(CGFloat)offsetX rightLabel:(GFDisplayTitleLabel *)rightLabel leftLabel:(GFDisplayTitleLabel *)leftLabel
{
    if (_isShowTitleGradient == NO) return;

    // 获取右边缩放
    CGFloat rightSacle = offsetX / GFScreenW - leftLabel.tag;

    // 获取左边缩放比例
    CGFloat leftScale = 1 - rightSacle;

    // RGB渐变
    if (_titleColorGradientStyle == GFTitleColorGradientStyleRGB) {
    ...
    // 填充渐变
    if (_titleColorGradientStyle == GFTitleColorGradientStyleFill) {
...
- (void)setUpTitleScaleWithOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    CGFloat scaleTransform = _titleScale?_titleScale:GFTitleTransformScale;
...
- (void)setUpUnderLineOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    // 获取两个标题中心点距离
    CGFloat centerDelta = rightLabel.gf_x - leftLabel.gf_x;
    ...
    // 计算当前下划线偏移量
    CGFloat underLineTransformX = offsetDelta * centerDelta / GFScreenW;

    // 宽度递增偏移量
    CGFloat underLineWidth = offsetDelta * widthDelta / GFScreenW;

    self.underLine.gf_width += underLineWidth;
    self.underLine.gf_x += underLineTransformX;
...
- (void)setUpCoverOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    // 计算当前下划线偏移量
    CGFloat coverTransformX = offsetDelta * centerDelta / GFScreenW;

    // 宽度递增偏移量
    CGFloat coverWidth = offsetDelta * widthDelta / GFScreenW;

    self.coverView.gf_width += coverWidth;
    self.coverView.gf_x += coverTransformX;
...
- (void)setSelectIndex:(NSInteger)selectIndex
{
    ...
    if (self.titleLabels.count) {
        ...
        if (_selectIndex >= self.titleLabels.count) {
            @throw [NSException exceptionWithName:@"GF_ERROR" reason:@"选中控制器的角标越界" userInfo:nil];
        }
...
- (void)titleClick:(UITapGestureRecognizer *)tap
{
    ...
    if (vc.view) {
        // 发出通知点击标题通知
        [[NSNotificationCenter defaultCenter] postNotificationName:GFDisplayViewClickOrScrollDidFinshNote  object:vc];

        // 发出重复点击标题通知
        if (_selIndex == i) {
            [[NSNotificationCenter defaultCenter] postNotificationName:GFDisplayViewRepeatClickTitleNote object:vc];
        }
    }
...
- (void)selectLabel:(UILabel *)label
{
    ...
    for (GFDisplayTitleLabel *labelView in self.titleLabels) {
        ...
        if (_isShowTitleGradient && _titleColorGradientStyle == GFTitleColorGradientStyleFill) {
...
- (void)setUpCoverView:(UILabel *)label
{
    ...
    self.coverView.gf_y = (label.gf_height - coverH) * 0.5;
    self.coverView.gf_height = coverH;


    // 最开始不需要动画
    if (self.coverView.gf_x == 0) {
        self.coverView.gf_width = coverW;

        self.coverView.gf_x = label.gf_x - border;
        return;
    }
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    CGFloat underLineH = _underLineH?_underLineH:GFUnderLineH;

    self.underLine.gf_y = label.gf_height - underLineH;
    self.underLine.gf_height = underLineH;


    // 最开始不需要动画
    if (self.underLine.gf_x == 0) {
        if (_isUnderLineEqualTitleWidth) {
            self.underLine.gf_width = titleBounds.size.width;
        } else {
            self.underLine.gf_width = label.gf_width;
        }

        self.underLine.gf_centerX = label.gf_centerX;
        return;
    }
...
- (void)setLabelTitleCenter:(UILabel *)label
{
    ...
    // 设置标题滚动区域的偏移量
    CGFloat offsetX = label.center.x - GFScreenW * 0.5;
    ...
    // 计算下最大的标题视图滚动区域
    CGFloat maxOffsetX = self.titleScrollView.contentSize.width - GFScreenW + _titleMargin;
...
- (void)refreshDisplay
{
    if (self.childViewControllers.count == 0) {
        @throw [NSException exceptionWithName:@"GF_ERROR" reason:@"请确定添加了所有子控制器" userInfo:nil];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    vc.view.frame = CGRectMake(0, 0, self.contentScrollView.gf_width, self.contentScrollView.gf_height);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    NSInteger screenWInt = GFScreenW;

    NSInteger extre = offsetXInt % screenWInt;
    if (extre > GFScreenW * 0.5) {
        // 往右边移动
        offsetX = offsetX + (GFScreenW - extre);
        _isAniming = YES;
        [self.contentScrollView setContentOffset:CGPointMake(offsetX, 0) animated:YES];
    }else if (extre < GFScreenW * 0.5 && extre > 0){
        _isAniming = YES;
        // 往左边移动
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 获取左边角标
    NSInteger leftIndex = offsetX / GFScreenW;

    // 左边按钮
    GFDisplayTitleLabel *leftLabel = self.titleLabels[leftIndex];

    // 右边角标
    NSInteger rightIndex = leftIndex + 1;

    // 右边按钮
    GFDisplayTitleLabel *rightLabel = nil;
